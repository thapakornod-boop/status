...
export type EmployeeRole = 'sales' | 'wh' | 'transport' | 'admin' | 'head' | 'hr'
...
export type StepDef = {
  ...
  accessRoles: EmployeeRole[] // who is ALLOWED to submit this step (any one of these)
  fields: FieldDef[]
}
...
// step 1 (เลือกร้านค้า) เป็นหน้าที่ของ role นี้ต่อ type
export const STEP1_ACCESS_ROLE: Record<CaseType, EmployeeRole> = {
  sales: 'sales',
  transport: 'transport',
}
...
export const STEPS: Record<CaseType, StepDef[]> = {
  ...
  transport: [
    ...
    {
      title: 'ขนส่งรับสินค้า / นำเข้าคลัง / หัวหน้าคลังเซ็น',
      role: 'ขนส่ง',
      accessRoles: ['transport'],
      fields: [
        { key: 'srn_signed_photo_url', label: 'รูปร้านค้าเซ็นใน SRN (ตัวบรรจง)', type: 'photo', required: true, camera: true },
...
export function getQueueStepNumbers(type: CaseType, role: EmployeeRole): number[] {
  ...
  return STEPS[type]
    .map((s, i) => ({ stepNumber: i + 2, accessRoles: s.accessRoles }))
    .filter((s) => s.accessRoles.includes(role))
    .map((s) => s.stepNumber)
}
...
export function canActOnStep(type: CaseType, stepNumber: number, role: EmployeeRole): boolean {
  ...
  if (stepNumber === 1) return role === STEP1_ACCESS_ROLE[type]
  const step = STEPS[type][stepNumber - 2]
  return step?.accessRoles.includes(role) ?? false
}