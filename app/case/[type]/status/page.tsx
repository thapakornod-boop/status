...
import { getQueueStepNumbers, ROLE_LABEL, EmployeeRole, CaseType as SharedCaseType } from '@/lib/caseSteps'
...
export default function CasePage() {
  ...
  // งานที่รอ role ของฉันทำ (ไม่ใช่แค่ที่ฉันสร้างเอง เช่น role wh/admin รับต่อจาก sales)
  const [queueCases, setQueueCases] = useState<MyCase[]>([])
  ...
  // งานที่รอ role ของฉันทำ (queue ตาม step ที่ role นี้รับผิดชอบ)
  const queueStepNumbers = useMemo(
    () => (employee ? getQueueStepNumbers(type as SharedCaseType, employee.role) : []),
    [employee, type]
  )

  useEffect(() => {
    if (!config || !employee || queueStepNumbers.length === 0) {
      setQueueCases([])
    ...
    supabase
      .from(config.table)
      .select('id, case_number, current_step, stores(store_code, store_name)')
      .in('current_step', queueStepNumbers)
      .neq('status', 'completed')
      .order('created_at', { ascending: true })
      .then(({ data }) => {
        setQueueCases((data as unknown as MyCase[]) ?? [])
        setLoadingQueue(false)
      })
  }, [config, employee, queueStepNumbers])
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-0 py-8">
        {/* หัวหน้า (head) เห็นแค่ปุ่มไปหน้าติดตามสถานะภาพรวม ไม่ต้องเห็นฟอร์ม */}
        {isHead ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
            <p className="text-lg font-semibold text-gray-800 mb-1">โหมดหัวหน้า — ดูภาพรวม</p>
            <p className="text-sm text-gray-400 mb-6">
              ดูสถานะทุกเคส ทุกขั้นตอน แบบ real-time
            </p>
            <button
              onClick={() => router.push(`/case/${type}/status`)}
          ...
          <>
            ...
            {tab === 'form' ? (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                {/* งานที่รอ role ของฉันทำ (เช่น wh รอรับต่อจาก sales, admin รอออก CN) */}
                {!loadingQueue && queueCases.length > 0 && (
                  <div className="mb-6">
                    <label className="block text-sm font-medium text-gray-600 mb-1">
                      🔔 งานที่รอคุณทำ ({queueCases.length})
                    </label>
                    <select
                      defaultValue=""
                      onChange={(e) => {
                ...
                {/* เคสที่ตัวเองเป็นคนสร้าง ทำค้างไว้ */}
